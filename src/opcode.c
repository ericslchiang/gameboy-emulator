...
void LD_HL_SP(void) {
    ...
    cpu.zero = 0;
    cpu.sub = 0;
    cpu.carry = (val & 0xFF) + (before & 0xFF) > 0xFF; //Overflow from bit 7 
    cpu.halfCarry = (val & 0xF) + (before & 0xF) > 0xF; //Overflow from bit 3
}
...
void ADD8(uint8_t *reg, uint8_t val) {
    ...
    *reg += val;
    ...
    cpu.zero = (*reg == 0);
    cpu.sub = 0;
    cpu.carry = (before + val > 0xFF); // Overflow on bit 7
    cpu.halfCarry = (before & 0xF) + (val & 0xF) > 0xF; // Overflow on bit 3
}
...
void SUB(uint8_t *reg, uint8_t val) {
    ...
    *reg -= val;
    ...
    cpu.zero = (*reg == 0);
    cpu.sub = 1;
    cpu.carry = val > before; // Check for byte underflow
    cpu.halfCarry = (val & 0xF) > (before & 0xF); // Check for half byte underflow
}
...
void ADD_SP(void) {
    ...
    cpu.zero = 0;
    cpu.sub = 0;
    cpu.carry = ((before & 0xFF) + val) > 0xFF; //Check for overflow on 7th bit
    // cpu.halfCarry = (val & 0xF) + (before & 0xF) > 0xF; //Overflow from bit 3
    if (val > 0) { //Check for overflow on 3rd bit
        uint8_t temp = val;
        cpu.halfCarry = ((before & 0xF) + (temp & 0xF)) > 0xF;
    } else {
        cpu.halfCarry = 0;
    }
}
...
void ADD16(uint16_t val) {
    ...
    cpu.sub = 0;
    cpu.carry = before + val > 0xFFFF; // Overflow on 15th bit
    cpu.halfCarry = (before & 0xFFF) + (val & 0xFFF) > 0xFFF; //Check for overflow on 11th bit    
}
...
void ADC(uint8_t *reg, uint8_t val) {
    ...
    *reg += val + cpu.carry;
    ...
    cpu.zero = (*reg == 0);
    cpu.sub = 0; 
    cpu.halfCarry = (before & 0xF) + (val & 0xF) + cpu.carry > 0xF;
    cpu.carry = before + val + cpu.carry > 0xFF;
}
...
void SBC(uint8_t *reg, uint8_t val) {
    ...
    *reg -= val + cpu.carry;
    ...
    cpu.zero = (*reg == 0);
    cpu.sub = 1; 
    cpu.halfCarry = (val & 0xF) + cpu.carry > (before & 0xF);
    cpu.carry = val + cpu.carry > before;
}
...
void CP(uint8_t *reg, uint8_t val){
    ...
    cpu.zero = (before == val);
    cpu.sub = 1; 
    cpu.halfCarry = (val & 0xF) > (before & 0xF);
    cpu.carry = val > before;
}
...
void RL(uint8_t *reg, uint8_t isCircular, uint8_t isRegA) {
    ...
    if (isCircular) {
        ...
        *reg = (before << 1) + cpu.carry; // Register rotated by 1 bit
    } else {
        *reg = (before << 1) + cpu.carry; // Register rotated by 1 bit including carry flag  
        cpu.carry = before >> 7; // Carry flag set to bit 7 of register
    }

    cpu.zero = (*reg == 0) && (isRegA == 0);
    cpu.sub = 0;
    cpu.halfCarry = 0;
}
...
void POP_AF(void) {
    cpu.f = memoryRead(cpu.sp++); 
    cpu.a = memoryRead(cpu.sp++);
    cpu.padding = 0;
}
...
void PUSH_AF(void) {
    memoryWrite(--cpu.sp, cpu.a); //Decrement stack address, write MSByte to stack
    memoryWrite(--cpu.sp, (cpu.zero << 7 | cpu.sub << 6 | cpu.halfCarry << 5 | cpu.carry << 4)); //Decrement stack address, write LSByte to stack
}

void CCF(void) {
    cpu.carry = 1 - cpu.carry; // Complement carry flag
    cpu.sub = 0;
    cpu.halfCarry = 0;
}
...
static uint8_t checkConditionCode(enum ConditionCode cc) {
    switch (cc) {
        case Z:
            return cpu.zero; break;
        case NZ:
            return cpu.zero ? 0 : 1; break;
        case C:
            return cpu.carry; break;
        case NC:
